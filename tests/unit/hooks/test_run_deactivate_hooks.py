...
def test_runs_configured_hooks(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_deactivate=["deactivate.sh"])

    hooks.run_on_deactivate_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH)

    assert _executables(mock_subprocess) == [str(BIN_PATH / "deactivate.sh")]


def test_hooks_execute_in_configured_order(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_deactivate=["first.sh", "second.sh", "third.sh"])
...
def test_skips_hooks_when_skip_hooks_is_true(mock_subprocess: MagicMock) -> None:
    ...
    hooks.run_on_deactivate_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH, skip_hooks=True)
...
def test_no_hooks_configured_runs_nothing(mock_subprocess: MagicMock) -> None:
    hooks.run_on_deactivate_hooks(ROOT, WORKTREE_PATH, Hooks(), branch=BRANCH)
