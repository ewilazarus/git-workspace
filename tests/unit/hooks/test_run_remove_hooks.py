...
def test_runs_configured_hooks(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_remove=["teardown.sh", "notify.sh"])

    hooks.run_on_remove_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH)
...
def test_hooks_execute_in_configured_order(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_remove=["first.sh", "second.sh", "third.sh"])
...
def test_skips_when_skip_hooks_is_true(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_remove=["teardown.sh"])

    hooks.run_on_remove_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH, skip_hooks=True)

    mock_subprocess.assert_not_called()


def test_hook_failure_raises_error(mock_subprocess: MagicMock) -> None:
...
def test_no_hooks_runs_nothing(mock_subprocess: MagicMock) -> None:
    hooks.run_on_remove_hooks(ROOT, WORKTREE_PATH, Hooks(), branch=BRANCH)
