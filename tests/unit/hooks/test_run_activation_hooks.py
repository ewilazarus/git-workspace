...
def test_runs_on_activate_hooks(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_activate=["activate.sh"])

    hooks.run_on_activate_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH)
...
def test_hooks_execute_in_configured_order(mock_subprocess: MagicMock) -> None:
    hook_config = Hooks(on_activate=["first.sh", "second.sh", "third.sh"])
...
def test_skips_hooks_when_skip_hooks_is_true(mock_subprocess: MagicMock) -> None:
    ...
    hooks.run_on_activate_hooks(ROOT, WORKTREE_PATH, hook_config, branch=BRANCH, skip_hooks=True)
...
def test_hook_failure_stops_execution(mock_subprocess: MagicMock) -> None:
    ...
    hook_config = Hooks(on_activate=["fails.sh", "never_runs.sh"])
...
def test_no_hooks_configured_runs_nothing(mock_subprocess: MagicMock) -> None:
    hooks.run_on_activate_hooks(ROOT, WORKTREE_PATH, Hooks(), branch=BRANCH)
