...
def test_runs_on_setup_hooks(mock_subprocess: MagicMock) -> None:
    ...
    workspace.run_on_setup_hooks_for_reset(ROOT, WORKTREE_PATH, hooks, branch=BRANCH)
...
def test_skips_hooks_when_skip_hooks_is_true(mock_subprocess: MagicMock) -> None:
    ...
    workspace.run_on_setup_hooks_for_reset(ROOT, WORKTREE_PATH, hooks, branch=BRANCH, skip_hooks=True)
