...
def test_runs_on_activate_for_new_worktree(mock_subprocess: MagicMock) -> None:
    hooks = Hooks(on_activate=["activate.sh"])

    workspace.run_on_activate_hooks(ROOT, NEW_RESULT, hooks, branch=BRANCH)

    assert _executables(mock_subprocess) == [str(BIN_PATH / "activate.sh")]


def test_runs_on_activate_for_existing_worktree(mock_subprocess: MagicMock) -> None:
    hooks = Hooks(on_activate=["activate.sh"])

    workspace.run_on_activate_hooks(ROOT, EXISTING_RESULT, hooks, branch=BRANCH)

    assert _executables(mock_subprocess) == [str(BIN_PATH / "activate.sh")]


def test_hooks_execute_in_configured_order(mock_subprocess: MagicMock) -> None:
    hooks = Hooks(on_activate=["first.sh", "second.sh", "third.sh"])
...
def test_skips_hooks_when_skip_hooks_is_true(mock_subprocess: MagicMock) -> None:
    ...
    workspace.run_on_activate_hooks(ROOT, NEW_RESULT, hooks, branch=BRANCH, skip_hooks=True)
...
def test_skips_hooks_when_skip_hooks_is_true_for_existing_worktree(mock_subprocess: MagicMock) -> None:
    ...
    workspace.run_on_activate_hooks(ROOT, EXISTING_RESULT, hooks, branch=BRANCH, skip_hooks=True)
...
def test_hook_failure_stops_execution(mock_subprocess: MagicMock) -> None:
    ...
    hooks = Hooks(on_activate=["fails.sh", "never_runs.sh"])
