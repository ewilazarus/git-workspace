...
def test_non_override_link_creates_symlink(workspace_with_links: Workspace) -> None:
    up(branch="main", workspace_dir=str(workspace_with_links.directory))
    assert (workspace_with_links.directory / "main" / ".dotfile").is_symlink()


def test_non_override_symlink_points_to_asset(workspace_with_links: Workspace) -> None:
    up(branch="main", workspace_dir=str(workspace_with_links.directory))
    link = workspace_with_links.directory / "main" / ".dotfile"
...
def test_override_link_creates_symlink(workspace_with_links: Workspace) -> None:
    up(branch="main", workspace_dir=str(workspace_with_links.directory))
    assert (workspace_with_links.directory / "main" / "settings.json").is_symlink()


def test_override_symlink_points_to_asset(workspace_with_links: Workspace) -> None:
    up(branch="main", workspace_dir=str(workspace_with_links.directory))
    link = workspace_with_links.directory / "main" / "settings.json"
...
def test_non_override_link_is_added_to_exclude_file(
    ...
    expected_path = str(
        (workspace_with_links.directory / "main" / ".dotfile").absolute()
    )
...
def test_override_link_is_not_added_to_exclude_file(
    ...
    exclude_content = workspace_with_links.paths.ignore_file.read_text()
    settings_path = str(
        (workspace_with_links.directory / "main" / "settings.json").absolute()
    )
...
def test_links_applied_across_multiple_worktrees(
    ...
    up(
        ...
        base_branch="main",
        workspace_dir=str(workspace_with_links.directory),
    )
    assert (workspace_with_links.directory / "main" / ".dotfile").is_symlink()
    assert (
        workspace_with_links.directory / "feature" / "other" / ".dotfile"
    ).is_symlink()
...
def test_reset_reapplies_non_override_link(workspace_with_links: Workspace) -> None:
    ...
    reset(branch="main", workspace_dir=str(workspace_with_links.directory))
...
def test_cannot_link_to_symlink_pointing_elsewhere(
    ...
    link = workspace_with_links.directory / "main" / ".dotfile"
    link.unlink()
    other = workspace_with_links.directory / "other-target"
